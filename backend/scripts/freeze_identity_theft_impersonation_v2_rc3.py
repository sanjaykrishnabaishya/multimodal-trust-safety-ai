"""Freeze development-passing Identity Theft & Impersonation V2 RC3."""
...
DATA = ROOT / "datasets" / "development" / "identity_theft_impersonation_v2_rc3" / "development.csv"
REPORT = ROOT / "reports" / "evaluation" / "identity_theft_impersonation" / "v2_rc3_development" / "report.json"
SOURCES = {
    ...
    "source_snapshot/freeze_identity_theft_impersonation_v2_rc3.py": Path(__file__).resolve(),
...
GATE = {
    ...
    "minimum_identity_precision": 0.97,
    "minimum_identity_recall": 0.95,
...
def main() -> None:
    ...
    if len(rows) != 400 or len({row["text"] for row in rows}) != 400:
    ...
    try:
        ...
        for relative, source in SOURCES.items():
            ...
            artifacts.append({"relative_path": relative,
                              "size_bytes": target.stat().st_size,
                              "sha256": sha(target)})
        manifest = {
            "candidate": CANDIDATE,
            "component": "Identity Theft & Impersonation",
            ...
            "artifacts": artifacts,
            "independent_gate_frozen_before_holdout": GATE,
            ...
            "voice_identity_matching_used": False,
    ...
    print("IDENTITY THEFT & IMPERSONATION V2 RC3 FREEZE\n" + "=" * 60)
    ...
    print("RC2 independent examples used for RC3: False")
    ...
    print("V2 RC3 is frozen for one aggregate-only independent challenge.")
