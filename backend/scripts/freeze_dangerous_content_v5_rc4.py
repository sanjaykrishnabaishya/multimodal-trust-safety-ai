...
DEVELOPMENT_EVALUATOR_PATH = (
    BACKEND_DIRECTORY
    / "scripts"
    / "evaluate_dangerous_content_v5_rc4_development.py"
)
DEVELOPMENT_DIAGNOSTIC_PATH = (
    BACKEND_DIRECTORY
    / "scripts"
    / "diagnose_dangerous_content_v5_rc4_development.py"
)
...
RC3_CANDIDATE_DIRECTORY = CANDIDATE_PARENT / "dangerous-content-v4-rc3"
RC3_MANIFEST_PATH = RC3_CANDIDATE_DIRECTORY / "manifest.json"
RC3_VERDICT_PATH = (
    RC3_CANDIDATE_DIRECTORY / "independent_evaluation_verdict.json"
)
...
FORBIDDEN_PRE_FREEZE_PATHS = (
    ...
    / "datasets"
    / "evaluation"
    / "dangerous_content_v5_rc4_independent",
    ROOT_DIRECTORY
    / "reports"
    / "evaluation"
    / "dangerous_content"
...
def verify_rc3_dependency(rc3_manifest: dict[str, Any]) -> None:
    artifacts = artifact_map(rc3_manifest)
    checks = (
        (
            "source_snapshot/dangerous_content_v4_rc3_service.py",
            RC3_LIVE_SERVICE_PATH,
        ),
        ("model/classifier_bundle.joblib", RC3_LIVE_MODEL_PATH),
        ("model/config.json", RC3_LIVE_CONFIG_PATH),
    )
    for relative_path, live_path in checks:
        frozen_path = RC3_CANDIDATE_DIRECTORY / relative_path
        record = artifacts.get(relative_path)
        if record is None:
            raise RuntimeError(f"RC3 dependency artifact is missing: {relative_path}")
        require_file(frozen_path)
        require_file(live_path)
        frozen_hash = sha256_file(frozen_path)
        if frozen_hash != record.get("sha256"):
            raise RuntimeError(f"RC3 frozen artifact changed: {relative_path}")
        if sha256_file(live_path) != frozen_hash:
            raise RuntimeError(f"RC3 live dependency differs from freeze: {relative_path}")
...
def main() -> None:
    ...
    if CANDIDATE_DIRECTORY.exists():
        raise FileExistsError(
            "A frozen RC4 candidate already exists and will not be overwritten: "
    ...
    rc3_manifest = read_json(RC3_MANIFEST_PATH)
    rc3_verdict = read_json(RC3_VERDICT_PATH)
    verify_rc3_dependency(rc3_manifest)

    failures: list[str] = []
    if report.get("candidate") != "dangerous-content-v5-rc4-development":
        failures.append("Unexpected RC4 development candidate identity.")
    if report.get("passed_development_gate") is not True:
        failures.append("The RC4 development gate did not pass.")
    metric_minimums = {
        "accuracy": 0.90,
    ...
    for key in (
        ...
        "processing_errors",
        "rc3_development_text_overlap",
    ):
        if int(report.get(key, -1)) != 0:
            failures.append(f"Development contract is nonzero: {key}.")
    for key in (
        "rc3_holdout_cases_read",
        "rc3_holdout_predictions_or_mismatches_read",
    ...
    if report.get("rc3_independent_aggregate_used_as_development_signal") is not True:
        failures.append("RC3 aggregate signal usage was not documented.")
    if dataset_manifest.get("candidate") != report.get("candidate"):
        failures.append("The RC4 dataset and report identities differ.")
    if rc3_manifest.get("candidate") != "dangerous-content-v4-rc3":
        failures.append("Unexpected RC3 dependency identity.")
    if rc3_verdict.get("passed_synthetic_independent_readiness_gate") is not False:
        failures.append("RC3 dependency verdict is not the recorded failed state.")
    if failures:
        raise RuntimeError(
            "Dangerous Content V5 RC4 freeze validation failed:\n- "
            + "\n- ".join(failures)
        )

    CANDIDATE_PARENT.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="dangerous-content-v5-rc4-freeze-",
        ...
        artifacts = [
            ...
            copy_artifact(
                DEVELOPMENT_DIAGNOSTIC_PATH,
                temporary / "source_snapshot" / DEVELOPMENT_DIAGNOSTIC_PATH.name,
                f"source_snapshot/{DEVELOPMENT_DIAGNOSTIC_PATH.name}",
            ),
            copy_artifact(
                FREEZE_SCRIPT_PATH,
            ...
            copy_artifact(
                RC3_MANIFEST_PATH,
                temporary / "dependency_snapshot" / "rc3_manifest.json",
                "dependency_snapshot/rc3_manifest.json",
            ),
            copy_artifact(
                RC3_VERDICT_PATH,
                temporary / "dependency_snapshot" / "rc3_verdict.json",
                "dependency_snapshot/rc3_verdict.json",
            ),
            copy_artifact(
                RC3_LIVE_SERVICE_PATH,
                temporary
                / "dependency_snapshot"
                / RC3_LIVE_SERVICE_PATH.name,
                f"dependency_snapshot/{RC3_LIVE_SERVICE_PATH.name}",
            ),
            copy_artifact(
                RC3_LIVE_MODEL_PATH,
                temporary
                / "dependency_snapshot"
                / RC3_LIVE_MODEL_PATH.name,
                f"dependency_snapshot/{RC3_LIVE_MODEL_PATH.name}",
            ),
            copy_artifact(
        ...
        manifest = {
            ...
            "candidate_type": (
                "frozen RC3 semantic policy plus guarded direct-advocacy recovery"
            ),
            ...
            "development_metrics": {
                ...
                for key in (
                    ...
                    "rc3_semantic_accepts",
                    "direct_advocacy_recoveries",
                    "action_contract_failures",
                    "category_mix_failures",
                    "processing_errors",
                )
            ...
            "base_dependency": {
                ...
                "source_and_artifact_hashes_verified": True,
                "independent_gate_passed": False,
                "used_only_inside_rc4_guards": True,
            },
            "rc3_aggregate_used_as_development_signal": True,
            ...
            "direct_recovery_requires_advocacy_and_hazard_evidence": True,
    ...
    print("DANGEROUS CONTENT V5 RC4 FREEZE")
    print("=" * 60)
    ...
    print("Frozen RC3 dependency hashes verified: True")
    print("RC3 independent aggregate used as development signal: True")
    ...
    print("Direct recovery requires advocacy and hazard evidence: True")
