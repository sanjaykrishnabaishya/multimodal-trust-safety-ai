"""Freeze development-passing Identity Theft & Impersonation V2 RC5."""
...
CANDIDATE = "identity-theft-impersonation-v2-rc5"
DEST = BACKEND / "storage/candidates" / CANDIDATE
DATA = ROOT / "datasets/development/identity_theft_impersonation_v2_rc5/development.csv"
REPORT = ROOT / "reports/evaluation/identity_theft_impersonation/v2_rc5_development/report.json"
SOURCES = {
    ...
    **{f"dependency_snapshot/identity_theft_impersonation_v2_rc{n}_policy.json": BACKEND / f"app/evidence/identity_theft_impersonation_v2_rc{n}_policy.json" for n in (1, 2, 3, 4)},
    ...
    "source_snapshot/freeze_identity_theft_impersonation_v2_rc5.py": Path(__file__).resolve(),
...
def main() -> None:
    ...
    required = {
        "passed_development_gate": True, "records": 500, "unique_texts": 500,
    ...
    with DATA.open("r", encoding="utf-8", newline="") as handle: rows = list(csv.DictReader(handle))
    if len(rows) != 500 or len({row["text"] for row in rows}) != 500: raise RuntimeError("Dataset uniqueness failed")
    temporary = DEST.with_name(DEST.name + ".tmp"); temporary.mkdir(parents=True)
    try:
        artifacts = []
        for relative, source in SOURCES.items():
            target = temporary / relative; target.parent.mkdir(parents=True, exist_ok=True); shutil.copy2(source, target)
        ...
        manifest = {
            ...
            "frozen_at_utc": datetime.now(timezone.utc).isoformat(), "development_gate_passed": True,
            "development_metrics": {key: report[key] for key in (
                ...
                "category_mix_failures", "data_contract_failures", "authority_contract_failures", "processing_errors")},
            ...
            "prior_independent_examples_used_for_rc5": False,
            "prior_individual_predictions_used_for_rc5": False,
            "real_identity_documents_used": False, "raw_credentials_used": False,
            "complete_personal_identifiers_used": False, "face_recognition_used": False,
            "voice_identity_matching_used": False, "biometric_embeddings_used": False,
            ...
            "eligible_for_guarded_live_integration": False, "connected_to_live_moderation": False,
            "automatic_account_suspension_allowed": False, "automatic_enforcement_allowed": False,
            "evaluation_reporting_contract": {"store_raw_holdout_text": False,
                "store_individual_predictions": False, "print_individual_predictions": False,
        ...
        raise
    print("IDENTITY THEFT & IMPERSONATION V2 RC5 FREEZE\n" + "=" * 60)
