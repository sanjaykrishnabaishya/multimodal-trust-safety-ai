...
from scripts.evaluate_dangerous_content_v6_rc5_live_fusion_contract_v3 import (
...
REPORT_PATH = (
    ...
    / "evaluation"
    / "dangerous_content"
    / "v6_rc5_live_fusion_contract_v4"
    / "report.json"
)


def build_cases():
    cases = deepcopy(build_v3_cases())
    for case in cases:
        if case["case_id"] == "GRAPHIC-DEPICTION-BOUNDARY-001":
            case["allowed_categories"] = ["Uncertain"]
            case["require_review"] = True
            case["require_safe_pair"] = True
    return cases


def main() -> None:
    print("Running the Dangerous Content V6 RC5 live fusion contract V4...")
    ...
    report = {
        "contract": "dangerous-content-v6-rc5-live-fusion-v4",
        "records": len(rows),
        "passed": passed,
        ...
        "graphic_depiction_fails_closed_to_review": True,
        "rows": rows,
    }
    ...
    print("\n" + "=" * 60)
    print("DANGEROUS CONTENT V6 RC5 LIVE FUSION CONTRACT V4")
    print("=" * 60)
    ...
    print(
        "V4 fails closed: a pure graphic violence depiction becomes "
        "Uncertain and is referred to human review until the Violence "
        "component passes independent readiness."
    )
