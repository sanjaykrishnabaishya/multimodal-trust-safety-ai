from __future__ import annotations

import csv
import hashlib
import json
import re
import shutil
import unicodedata
from collections import defaultdict
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

import joblib
import numpy as np
from sentence_transformers import SentenceTransformer


ROOT = Path(__file__).resolve().parents[2]
BASE_CANDIDATE_NAME = "religiously-offensive-v6-rc5"
BASE_CANDIDATE = ROOT / "backend" / "storage" / "candidates" / BASE_CANDIDATE_NAME
BASE_MANIFEST = BASE_CANDIDATE / "manifest.json"
BASE_VERDICT = BASE_CANDIDATE / "independent_evaluation_verdict.json"
BASE_MODEL = BASE_CANDIDATE / "model" / "classifier.joblib"
BASE_CONFIG = BASE_CANDIDATE / "model" / "config.json"
RC5_AUDIT = (
    ROOT
    / "reports"
    / "evaluation"
    / "religiously_offensive"
    / "v6_rc5_pre_holdout_policy_contract"
    / "report.json"
)
BASE_VALIDATION = (
    ROOT
    / "datasets"
    / "development"
    / "religiously_offensive_v4_rc3"
    / "validation.csv"
)
BASE_TRAIN = BASE_VALIDATION.parent / "train.csv"

CANDIDATE = "religiously-offensive-v7-rc6-development"
MODEL_DIRECTORY = (
    ROOT / "backend" / "storage" / "models" / "religiously_offensive_v7_rc6"
)
MODEL_ARTIFACT = MODEL_DIRECTORY / "classifier.joblib"
MODEL_CONFIG = MODEL_DIRECTORY / "config.json"
DEVELOPMENT_DATASET_DIRECTORY = (
    ROOT / "datasets" / "development" / "religiously_offensive_v7_rc6"
)
EXPANSION_DATASET = DEVELOPMENT_DATASET_DIRECTORY / "policy_expansion.csv"
REPORT_DIRECTORY = (
    ROOT
    / "reports"
    / "evaluation"
    / "religiously_offensive"
    / "v7_rc6_dual_evidence_development"
)
REPORT_PATH = REPORT_DIRECTORY / "validation_report.json"

RELIGIOUS_LABEL = "religiously_offensive"
NO_OVERRIDE = "no_religious_override"
POLICY_GATE_VERSION = "religious-dual-evidence-gate-2026.08-v2"


SAFE_CONTEXT_PATTERN = re.compile(
    r"\b(?:"
    r"news|journalist|journalism|article|report|reported|reporting|"
    r"documentary|historian|history|historical|archive|museum|catalogue|"
    r"court|legal|police|research|researcher|study|academic|scholar|"
    r"lesson|classroom|educational|workshop|seminar|"
    r"public[- ]service|safety campaign|prevention|restoration|restored|"
    r"repair|repaired|conservator|interfaith|cultural context|"
    r"good[- ]faith criticism|respectful criticism|respectfully critiques?|"
    r"as evidence|provides? context"
    r")\b",
    re.IGNORECASE,
)
STRONG_SAFE_PATTERN = re.compile(
    r"\b(?:"
    r"condemn(?:s|ed|ing)?|oppose(?:s|d|ing)?|reject(?:s|ed|ing)?|"
    r"disavow(?:s|ed|ing)?|denounce(?:s|d|ing)?|"
    r"does not endorse|did not endorse|without endorsing|"
    r"warn(?:s|ed|ing)? against|discourage(?:s|d|ing)?|"
    r"prevent(?:s|ed|ing)?|stop(?:s|ped|ping)?"
    r")\b",
    re.IGNORECASE,
)
REPORTED_CONTEXT_PATTERN = re.compile(
    r"\b(?:news (?:article|report|story)|journalistic (?:article|report)|"
    r"court (?:report|record|summary|transcript)|legal (?:report|record|summary)|"
    r"police (?:report|record|statement)|documentary|historical (?:record|account)|"
    r"museum (?:program|exhibit|catalogue)|research (?:paper|report|study))\b"
    r".{0,160}\b(?:quote(?:s|d|ing)?|record(?:s|ed|ing)?|recount(?:s|ed|ing)?|"
    r"report(?:s|ed|ing)?|describe(?:s|d|ing)?|document(?:s|ed|ing)?|"
    r"mention(?:s|ed|ing)?|examine(?:s|d|ing)?|discuss(?:es|ed|ing)?)\b",
    re.IGNORECASE,
)
NEGATED_HARM_PATTERN = re.compile(
    r"\b(?:do not|don't|never|must not|should not|shouldn't|stop|avoid|prevent)\b"
    r".{0,120}\b(?:burn|destroy|damage|desecrate|deface|smash|mock|insult|"
    r"ridicule|vandalize|vandalise|demolish|profane|humiliate)\b",
    re.IGNORECASE,
)
DIRECT_ADVOCACY_PATTERN = re.compile(
    r"\b(?:"
    r"call(?:s|ed|ing)? (?:on|for)|"
    r"urg(?:e|es|ed|ing)|"
    r"ask(?:s|ed|ing)? (?:people|listeners|participants|followers|viewers|users)|"
    r"encourag(?:e|es|ed|ing)|order(?:s|ed|ing)?|demand(?:s|ed|ing)?|"
    r"command(?:s|ed|ing)?|promot(?:e|es|ed|ing)|endors(?:e|es|ed|ing)|"
    r"prais(?:e|es|ed|ing)|celebrat(?:e|es|ed|ing)|plan(?:s|ned|ning)?|"
    r"vow(?:s|ed|ing)?|threaten(?:s|ed|ing)?|want(?:s|ed|ing)?|"
    r"should|must|need to|let us|join us"
    r")\b",
    re.IGNORECASE,
)
FOLLOWER_REFERENCE_PATTERN = re.compile(
    r"\b(?:followers?|adherents?|devotees?|believers?|worshippers?|"
    r"congregants?|members?|families|workers|students|people|community|group)\b"
    r".{0,80}\b(?:religion|religious|faith|sect|denomination)\b|"
    r"\b(?:religion|religious|faith|sect|denomination)\b"
    r".{0,80}\b(?:followers?|adherents?|devotees?|believers?|worshippers?|"
    r"congregants?|members?|families|workers|students|people|community|group)\b",
    re.IGNORECASE,
)
FOLLOWER_HARM_PATTERN = re.compile(
    r"\b(?:inferior|parasites?|subhuman|not human|less than human|criminals?|"
    r"dangerous|exclude|expel|segregate|deny|refuse|remove|ban|boycott|"
    r"attack|harm|evict|strip|lose .*?rights?|forced? to leave|driven away|"
    r"should not exist|unfit for citizenship)\b",
    re.IGNORECASE,
)
OBJECT_OR_PLACE_PATTERN = re.compile(
    r"\b(?:holy|sacred|religious|devotional|consecrated|ceremonial|revered)\s+"
    r"(?:book|text|scripture|manuscript|scroll|emblem|symbol|icon|image|"
    r"portrait|painting|statue|sculpture|relic|object|artefact|artifact|"
    r"prayer beads|worship artefact|worship artifact|vessel|banner|flag|"
    r"monument|carving|artwork|site|sanctuary|centre|center|hall|building)|"
    r"\b(?:temple|church|mosque|gurudwara|gurdwara|shrine|chapel|monastery|"
    r"sanctuary|prayer beads|worship artefact|worship artifact|altar|"
    r"prayer hall|prayer space|place of worship|worship building|"
    r"worship centre|worship center|religious building|religious centre|"
    r"religious center)\b",
    re.IGNORECASE,
)
FIGURE_PATTERN = re.compile(
    r"\b(?:deity|deities|god|gods|prophet|prophets|faith founder|"
    r"religious founder|religious figure|religious figurehead|spiritual leader|"
    r"religious leader|faith leader|sacred figure|divine figure|holy person|"
    r"revered teacher|religious teacher|spiritual guide)\b",
    re.IGNORECASE,
)
PHYSICAL_ATTACK_PATTERN = re.compile(
    r"\b(?:burn|burned|burning|incinerate|scorch|torch|set .*?ablaze|"
    r"set .*?fire|destroy|damage|desecrate|profane|deface|disfigure|smash|"
    r"shatter|break|crush|tear|rip|shred|cut|slash|hack|trample|stomp|kick|"
    r"drag|spit|soil|smear|coat .*?filth|throw .*?(?:fire|sewer|garbage)|"
    r"vandalize|vandalise|mutilate|dismantle|demolish|bulldoze|raze|level|"
    r"wreck|ransack|obliterate|spray .*?(?:obscene|degrading)|"
    r"paint .*?(?:obscene|degrading)|cover .*?(?:obscene|degrading))\b",
    re.IGNORECASE,
)
FIGURE_ATTACK_PATTERN = re.compile(
    r"\b(?:mock|ridicule|vilify|demean|humiliate|insult|degrade|revile|deride|"
    r"slander|lampoon|denigrate|abuse|obscene|vile|degrading|abusive|"
    r"repulsive|morphed|caricature|parody|contemptuous)\b",
    re.IGNORECASE,
)


def sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def normalize(text: str) -> str:
    return " ".join(
        unicodedata.normalize("NFKC", str(text or "")).casefold().split()
    )


def text_hash(text: str) -> str:
    return hashlib.sha256(normalize(text).encode("utf-8")).hexdigest()


def require_file(path: Path) -> None:
    if not path.is_file():
        raise FileNotFoundError(f"Required RC6 development file is missing: {path}")


def verify_base() -> tuple[dict[str, Any], dict[str, Any]]:
    for path in (
        BASE_MANIFEST,
        BASE_MODEL,
        BASE_CONFIG,
        RC5_AUDIT,
        BASE_TRAIN,
        BASE_VALIDATION,
    ):
        require_file(path)
    if BASE_VERDICT.exists():
        raise RuntimeError(
            "RC5 unexpectedly has an independent verdict. RC6 development refuses "
            "to consume a candidate after independent testing."
        )
    manifest = json.loads(BASE_MANIFEST.read_text(encoding="utf-8"))
    config = json.loads(BASE_CONFIG.read_text(encoding="utf-8"))
    audit = json.loads(RC5_AUDIT.read_text(encoding="utf-8"))
    if manifest.get("candidate") != BASE_CANDIDATE_NAME:
        raise RuntimeError("Unexpected RC5 base manifest.")
    if manifest.get("independent_holdout_created_before_freeze") is not False:
        raise RuntimeError("RC5 holdout chronology is invalid.")
    if manifest.get("independent_holdout_used_before_freeze") is not False:
        raise RuntimeError("RC5 holdout integrity is invalid.")
    if manifest.get("connected_to_live_moderation") is not False:
        raise RuntimeError("RC5 is already connected to live moderation.")
    if config.get("candidate") != "religiously-offensive-v6-rc5-development":
        raise RuntimeError("Unexpected RC5 model configuration.")
    if audit.get("candidate") != BASE_CANDIDATE_NAME:
        raise RuntimeError("Unexpected RC5 policy audit.")
    if audit.get("passed_pre_holdout_policy_contract") is not False:
        raise RuntimeError("RC6 is only permitted after the RC5 pre-holdout audit failed.")
    if audit.get("independent_holdout_created_or_used") is not False:
        raise RuntimeError("The RC5 audit used an independent holdout.")
    if audit.get("prior_holdout_records_read") is not False:
        raise RuntimeError("The RC5 audit read prior holdout records.")
    if audit.get("candidate_modified") is not False:
        raise RuntimeError("RC5 was modified during its policy audit.")
    for artifact in manifest.get("artifacts", []):
        path = BASE_CANDIDATE / str(artifact["relative_path"])
        require_file(path)
        if sha256_file(path) != str(artifact["sha256"]):
            raise RuntimeError(f"Frozen RC5 artifact hash mismatch: {path.name}")
    return manifest, config


def load_csv(path: Path) -> list[dict[str, str]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        return list(csv.DictReader(handle))


def strong_safe_or_boundary_veto(text: str) -> tuple[bool, str]:
    value = normalize(text)
    if STRONG_SAFE_PATTERN.search(value) or NEGATED_HARM_PATTERN.search(value):
        return True, "explicit_condemnation_negation_or_prevention"
    if REPORTED_CONTEXT_PATTERN.search(value):
        return True, "reported_or_documented_context"
    if SAFE_CONTEXT_PATTERN.search(value) and not DIRECT_ADVOCACY_PATTERN.search(value):
        return True, "safe_reporting_education_or_criticism"
    if FOLLOWER_REFERENCE_PATTERN.search(value) and FOLLOWER_HARM_PATTERN.search(value):
        return True, "protected_follower_hate_boundary"
    return False, ""


def sacred_attack_anchor(text: str) -> bool:
    value = normalize(text)
    object_or_place = bool(OBJECT_OR_PLACE_PATTERN.search(value))
    figure = bool(FIGURE_PATTERN.search(value))
    physical = bool(PHYSICAL_ATTACK_PATTERN.search(value))
    figure_attack = bool(FIGURE_ATTACK_PATTERN.search(value))
    return bool((object_or_place and physical) or (figure and (physical or figure_attack)))


def direct_advocacy_signal(text: str) -> bool:
    return bool(DIRECT_ADVOCACY_PATTERN.search(normalize(text)))


def semantic_religious_signal(
    classifier: Any,
    probabilities: np.ndarray,
    config: dict[str, Any],
) -> np.ndarray:
    classes = np.asarray(classifier.classes_)
    religious_index = int(np.where(classes == RELIGIOUS_LABEL)[0][0])
    policy = config["semantic_guard_policy"]
    threshold = float(policy["religious_probability_threshold"])
    minimum_margin = float(policy["minimum_religious_margin"])
    output: list[bool] = []
    for scores in probabilities:
        religious_score = float(scores[religious_index])
        competing_score = float(np.max(np.delete(scores, religious_index)))
        output.append(
            religious_score >= threshold
            and religious_score - competing_score >= minimum_margin
        )
    return np.array(output)


def build_policy_expansion() -> list[dict[str, str]]:
    direct_targets = [
        "burn the sacred manuscript",
        "vandalize the neighborhood shrine",
        "smash the holy statue",
        "deface the religious symbol",
        "tear the sacred scripture",
        "demolish the prayer hall",
        "desecrate the temple altar",
        "mock the revered deity with an obscene image",
        "ridicule the religious founder with a degrading caricature",
        "trample the ceremonial banner",
    ]
    direct_frames = [
        "The organizer called on the audience to {target}.",
        "An online group is urging its members to {target}.",
        "The account demanded that participants {target}.",
    ]
    rows: list[dict[str, str]] = []
    for frame_index, frame in enumerate(direct_frames):
        for target_index, target in enumerate(direct_targets):
            rows.append(
                {
                    "split": "development",
                    "label": RELIGIOUS_LABEL,
                    "family": f"direct_advocacy_frame_{frame_index}",
                    "text": frame.format(target=target),
                    "case_id": f"RC6-D-{frame_index:02d}-{target_index:02d}",
                }
            )

    reporting_frames = [
        "A news report quoted someone calling for people to {target}; the report condemned the statement.",
        "A court transcript recorded that the accused ordered others to {target}; it did not endorse the act.",
        "A documentary recounted a campaign urging viewers to {target} while clearly opposing the campaign.",
    ]
    for frame_index, frame in enumerate(reporting_frames):
        for target_index, target in enumerate(direct_targets):
            rows.append(
                {
                    "split": "development",
                    "label": NO_OVERRIDE,
                    "family": f"reported_advocacy_safe_frame_{frame_index}",
                    "text": frame.format(target=target),
                    "case_id": f"RC6-R-{frame_index:02d}-{target_index:02d}",
                }
            )

    follower_templates = [
        "Followers of that religion are inferior and should be excluded from public life.",
        "People belonging to that faith should lose their rights.",
        "Those religious believers are dangerous and must be expelled.",
        "Members of that faith community should be denied housing.",
        "Adherents of that religion are parasites and should be driven away.",
        "Religious worshippers from that denomination should be segregated.",
        "Workers of that faith should be refused employment.",
        "Students from that religious community should be banned from the school.",
        "Families following that religion are unfit for citizenship.",
        "Believers from that sect should not exist in this neighborhood.",
    ]
    for repetition in range(2):
        for index, text in enumerate(follower_templates):
            rows.append(
                {
                    "split": "development",
                    "label": NO_OVERRIDE,
                    "family": "protected_follower_hate_boundary",
                    "text": f"Message {repetition + 1}: {text}",
                    "case_id": f"RC6-F-{repetition:02d}-{index:02d}",
                }
            )

    neutral_templates = [
        "Visitors admired the sacred painting during the exhibition.",
        "The restored prayer hall reopened for a community gathering.",
        "A scholar translated the holy manuscript for an academic archive.",
        "The museum displayed a religious statue with historical context.",
        "The congregation repaired the church roof after the storm.",
        "Volunteers cleaned the temple courtyard before the ceremony.",
        "The documentary explains the history of a revered religious teacher.",
        "The library preserved the sacred text in a protective case.",
        "An interfaith group visited the mosque during an educational tour.",
        "A conservator restored the ceremonial icon without altering it.",
    ]
    for repetition in range(2):
        for index, text in enumerate(neutral_templates):
            rows.append(
                {
                    "split": "development",
                    "label": NO_OVERRIDE,
                    "family": "neutral_religious_context",
                    "text": f"Context {repetition + 1}: {text}",
                    "case_id": f"RC6-N-{repetition:02d}-{index:02d}",
                }
            )

    unrelated_templates = [
        "The manager threatened to smash an employee's phone.",
        "A user repeatedly insulted another person in a private chat.",
        "The advertisement promises a cash prize for sharing a password.",
        "The post publishes a private residential address.",
        "The article makes an unsupported claim about a public election.",
        "A stranger ordered someone to transfer money immediately.",
        "The video depicts a graphic assault on an unidentified person.",
        "The message contains a single personal insult with no religious target.",
        "The file includes malicious code designed to steal credentials.",
        "The seller distributed a copyrighted film without permission.",
    ]
    for repetition in range(2):
        for index, text in enumerate(unrelated_templates):
            rows.append(
                {
                    "split": "development",
                    "label": NO_OVERRIDE,
                    "family": "unrelated_category_boundary",
                    "text": f"Example {repetition + 1}: {text}",
                    "case_id": f"RC6-O-{repetition:02d}-{index:02d}",
                }
            )
    return rows


def safe_divide(numerator: int, denominator: int) -> float:
    return numerator / denominator if denominator else 0.0


def main() -> None:
    manifest, base_config = verify_base()
    validation_rows = load_csv(BASE_VALIDATION)
    train_rows = load_csv(BASE_TRAIN)
    expansion_rows = build_policy_expansion()
    existing_hashes = {
        text_hash(row["text"])
        for row in train_rows + validation_rows
    }
    expansion_hashes = [text_hash(row["text"]) for row in expansion_rows]
    if len(expansion_hashes) != len(set(expansion_hashes)):
        raise RuntimeError("The RC6 development expansion contains duplicate text.")
    overlap = existing_hashes.intersection(expansion_hashes)
    if overlap:
        raise RuntimeError(f"RC6 development expansion overlaps existing data: {len(overlap)}")

    DEVELOPMENT_DATASET_DIRECTORY.mkdir(parents=True, exist_ok=True)
    with EXPANSION_DATASET.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["split", "label", "family", "text", "case_id"],
        )
        writer.writeheader()
        writer.writerows(expansion_rows)

    combined_rows: list[dict[str, str]] = []
    for row in validation_rows:
        combined_rows.append(
            {
                "label": (
                    RELIGIOUS_LABEL
                    if row["label"] == RELIGIOUS_LABEL
                    else NO_OVERRIDE
                ),
                "family": f"base_{row['family']}",
                "text": row["text"],
            }
        )
    combined_rows.extend(expansion_rows)

    classifier = joblib.load(BASE_MODEL)
    encoder = SentenceTransformer(
        str(base_config["embedding_model"]),
        revision=str(base_config["embedding_model_revision"]),
        device="cpu",
    )
    texts = [row["text"] for row in combined_rows]
    truth = np.array([row["label"] == RELIGIOUS_LABEL for row in combined_rows])
    embeddings = encoder.encode(
        texts,
        batch_size=32,
        normalize_embeddings=True,
        show_progress_bar=True,
    )
    probabilities = classifier.predict_proba(embeddings)
    semantic = semantic_religious_signal(classifier, probabilities, base_config)
    anchors = np.array([sacred_attack_anchor(text) for text in texts])
    direct_advocacy = np.array([direct_advocacy_signal(text) for text in texts])
    vetoes = [strong_safe_or_boundary_veto(text) for text in texts]
    predicted = np.array(
        [
            bool(
                not vetoes[index][0]
                and anchors[index]
                and (semantic[index] or direct_advocacy[index])
            )
            for index in range(len(combined_rows))
        ]
    )

    tp = int(np.sum(truth & predicted))
    tn = int(np.sum(~truth & ~predicted))
    fp = int(np.sum(~truth & predicted))
    fn = int(np.sum(truth & ~predicted))
    precision = safe_divide(tp, tp + fp)
    recall = safe_divide(tp, tp + fn)
    specificity = safe_divide(tn, tn + fp)
    f1 = safe_divide(2 * precision * recall, precision + recall)
    accuracy = safe_divide(tp + tn, len(combined_rows))

    grouped_indices: dict[str, list[int]] = defaultdict(list)
    for index, row in enumerate(combined_rows):
        grouped_indices[row["family"]].append(index)
    family_results: dict[str, dict[str, Any]] = {}
    for family, indices in sorted(grouped_indices.items()):
        correct = sum(bool(predicted[index]) == bool(truth[index]) for index in indices)
        family_results[family] = {
            "records": len(indices),
            "correct": int(correct),
            "accuracy": round(correct / len(indices), 4),
        }
    minimum_family_accuracy = min(item["accuracy"] for item in family_results.values())

    reported_indices = [
        index
        for index, row in enumerate(combined_rows)
        if row["family"].startswith("reported_advocacy_safe_frame_")
    ]
    follower_indices = [
        index
        for index, row in enumerate(combined_rows)
        if "protected_follower" in row["family"] or "followers_validation" in row["family"]
    ]
    reported_safe_rate = safe_divide(
        sum(not predicted[index] for index in reported_indices),
        len(reported_indices),
    )
    follower_safe_rate = safe_divide(
        sum(not predicted[index] for index in follower_indices),
        len(follower_indices),
    )
    deterministic_recoveries = int(
        np.sum(truth & predicted & ~semantic & direct_advocacy)
    )
    category_mix_failures = fp
    passed = bool(
        accuracy >= 0.95
        and precision >= 0.98
        and recall >= 0.92
        and specificity >= 0.99
        and f1 >= 0.94
        and minimum_family_accuracy >= 0.85
        and reported_safe_rate == 1.0
        and follower_safe_rate == 1.0
        and category_mix_failures == 0
    )

    MODEL_DIRECTORY.mkdir(parents=True, exist_ok=True)
    REPORT_DIRECTORY.mkdir(parents=True, exist_ok=True)
    shutil.copy2(BASE_MODEL, MODEL_ARTIFACT)
    config = {
        "candidate": CANDIDATE,
        "base_candidate": BASE_CANDIDATE_NAME,
        "base_classifier_sha256": sha256_file(BASE_MODEL),
        "embedding_model": base_config["embedding_model"],
        "embedding_model_revision": base_config["embedding_model_revision"],
        "semantic_guard_policy": base_config["semantic_guard_policy"],
        "policy_gate_version": POLICY_GATE_VERSION,
        "policy_gate_source_sha256": sha256_file(Path(__file__).resolve()),
        "decision_paths": {
            "semantic_path": "semantic signal AND sacred-target attack anchor",
            "direct_advocacy_path": "direct advocacy AND sacred-target attack anchor",
            "shared_requirement": "no safe-context or protected-follower veto",
        },
        "strong_safe_veto_enabled": True,
        "reported_context_veto_enabled": True,
        "protected_follower_veto_enabled": True,
        "follower_boundary_output_enabled": False,
        "follower_boundary_owner": "Hate Speech & Discrimination V7",
        "permitted_active_output": "Religiously Offensive Content only",
        "development_metrics": {
            "records": len(combined_rows),
            "accuracy": round(accuracy, 4),
            "precision": round(precision, 4),
            "recall": round(recall, 4),
            "specificity": round(specificity, 4),
            "f1": round(f1, 4),
            "minimum_family_accuracy": minimum_family_accuracy,
            "reported_advocacy_safe_rate": round(reported_safe_rate, 4),
            "protected_follower_safe_rate": round(follower_safe_rate, 4),
            "category_mix_failures": category_mix_failures,
            "deterministic_recoveries": deterministic_recoveries,
        },
        "passed_development_gate": passed,
        "automatic_enforcement_allowed": False,
        "independently_validated": False,
        "connected_to_live_moderation": False,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
    }
    MODEL_CONFIG.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    report = {
        "candidate": CANDIDATE,
        "base_candidate": BASE_CANDIDATE_NAME,
        "base_validation_records": len(validation_rows),
        "policy_expansion_records": len(expansion_rows),
        "combined_records": len(combined_rows),
        "base_validation_sha256": manifest["datasets"]["validation"]["sha256"],
        "policy_expansion_sha256": sha256_file(EXPANSION_DATASET),
        "development_overlap": len(overlap),
        "accuracy": round(accuracy, 4),
        "religious_precision": round(precision, 4),
        "religious_recall": round(recall, 4),
        "religious_f1": round(f1, 4),
        "nonreligious_specificity": round(specificity, 4),
        "true_positives": tp,
        "true_negatives": tn,
        "false_positives": fp,
        "false_negatives": fn,
        "minimum_family_accuracy": minimum_family_accuracy,
        "reported_advocacy_safe_rate": round(reported_safe_rate, 4),
        "protected_follower_safe_rate": round(follower_safe_rate, 4),
        "category_mix_failures": category_mix_failures,
        "deterministic_recoveries": deterministic_recoveries,
        "family_results": family_results,
        "passed_development_gate": passed,
        "rc5_independent_holdout_created_or_used": False,
        "prior_holdout_records_read": False,
        "rc5_policy_audit_used_as_development_evidence": True,
        "classifier_retrained": False,
        "connected_to_live_moderation": False,
        "automatic_enforcement_allowed": False,
        "evidence_limit": "Development evidence, not independent accuracy.",
    }
    REPORT_PATH.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")

    print("RELIGIOUSLY OFFENSIVE CONTENT V7 RC6 DUAL-EVIDENCE DEVELOPMENT")
    print("=" * 60)
    print(f"Base validation records: {len(validation_rows)}")
    print(f"Policy expansion records: {len(expansion_rows)}")
    print(f"Combined records: {len(combined_rows)}")
    print(f"Development overlap: {len(overlap)}")
    print(f"Accuracy: {accuracy * 100:.2f}%")
    print(f"Religious precision: {precision * 100:.2f}%")
    print(f"Religious recall: {recall * 100:.2f}%")
    print(f"Religious F1: {f1 * 100:.2f}%")
    print(f"Nonreligious specificity: {specificity * 100:.2f}%")
    print(f"Minimum family accuracy: {minimum_family_accuracy * 100:.2f}%")
    print(f"Reported-advocacy safe routing: {reported_safe_rate * 100:.2f}%")
    print(f"Protected-follower safe routing: {follower_safe_rate * 100:.2f}%")
    print(f"Category-mix failures: {category_mix_failures}")
    print(f"Direct-advocacy recoveries beyond semantic path: {deterministic_recoveries}")
    print("Follower-boundary output enabled: False")
    print("Follower-boundary owner: Hate Speech & Discrimination V7")
    print()
    print(f"Passed development gate: {passed}")
    print(f"Artifact: {MODEL_ARTIFACT}")
    print(f"Configuration: {MODEL_CONFIG}")
    print(f"Policy expansion: {EXPANSION_DATASET}")
    print(f"Report: {REPORT_PATH}")
    print("RC5 independent holdout created or used: False")
    print("Prior holdout records read: False")
    print("Classifier retrained: False")
    print("Live moderation changed: False")
    print("This is development evidence, not independent accuracy.")


if __name__ == "__main__":
    main()
