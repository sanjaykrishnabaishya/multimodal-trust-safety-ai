"""Readiness-gated live fusion for frozen Identity V2 RC5."""
...
from app.services.identity_theft_impersonation_v2_rc5_service import (
    analyze_identity_theft_impersonation_v2_rc5,
    apply_identity_theft_impersonation_v2_rc5_fusion as apply_candidate,
)

BACKEND = Path(__file__).resolve().parents[2]
CANDIDATE_DIRECTORY = BACKEND / "storage/candidates/identity-theft-impersonation-v2-rc5"
...
@lru_cache(maxsize=1)
def get_identity_theft_impersonation_v2_rc5_readiness() -> dict[str, Any]:
    try:
        ...
        return {"ready": False, "reason": f"Candidate evidence unavailable: {type(error).__name__}",
                "automatic_enforcement_allowed": False}
    checks = {
        "candidate_identity": manifest.get("candidate") == "identity-theft-impersonation-v2-rc5",
        ...
        "frozen_live_and_dependency_hashes_verified": hashes_verified,
        "automatic_enforcement_disabled": manifest.get("automatic_enforcement_allowed") is False
            and verdict.get("automatic_enforcement_allowed") is False,
        "automatic_account_suspension_disabled": manifest.get("automatic_account_suspension_allowed") is False
            and verdict.get("automatic_account_suspension_allowed") is False,
        "sensitive_material_not_stored": manifest.get("real_identity_documents_used") is False
        ...
        "external_provider_disabled": manifest.get("external_provider_used") is False
            and manifest.get("external_transmission_allowed") is False
            and verdict.get("external_provider_used") is False,
        "biometric_and_appearance_inference_disabled": manifest.get("face_recognition_used") is False
    ...
    return {
        ...
        "reason": ("Frozen V2 RC5 passed its synthetic independent gate."
                   if ready else "Frozen V2 RC5 readiness contract is incomplete."),
...
def analyze_identity_theft_impersonation_v2_rc5_for_fusion(
    text: str, input_sources: list[str],
) -> dict[str, Any]:
    readiness = get_identity_theft_impersonation_v2_rc5_readiness()
    if not readiness.get("ready"):
        return {"available": False, "version": "identity-theft-impersonation-v2-rc5",
    ...
    if not TEXT_SOURCES.intersection(input_sources):
        return {"available": True, "version": "identity-theft-impersonation-v2-rc5",
    ...
    analysis = analyze_identity_theft_impersonation_v2_rc5(text, input_sources)
    analysis["readiness"] = readiness
    return analysis


def apply_identity_theft_impersonation_v2_rc5_guarded_fusion(
    *, category: str, severity: str, action: str, confidence: float,
    ...
    if not analysis.get("available") or not analysis.get("readiness", {}).get("ready"):
        return {"category": category, "severity": severity, "action": action,
                ...
                "automatic_enforcement_allowed": False}
    return apply_candidate(category=category, severity=severity, action=action,
                           confidence=confidence, human_review_required=human_review_required,
                           reason=reason, matched_signals=matched_signals, analysis=analysis)
